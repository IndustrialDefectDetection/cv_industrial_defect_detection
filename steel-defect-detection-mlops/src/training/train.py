...
from utils.mlflow_utils import MLflowTracker, log_yolov8_training
        ...
        if self.use_mlflow:
            try:
                self.mlflow_tracker = MLflowTracker(
                    experiment_name="steel-defect-detection",
                    model_name="steel-defect-yolov8"
                )
        ...
        try:
            ...
            if self.use_mlflow and self.mlflow_tracker:
                run_id = self.mlflow_tracker.start_run(
                    run_name=f"{self.run_name}_{device}",
                    tags={
                        "model_variant": self.model_name,
                        "device": str(device),
                        "dataset": "neu_steel_defects"
                    }
                )
                ...
                self.mlflow_tracker.log_training_params(training_config)
