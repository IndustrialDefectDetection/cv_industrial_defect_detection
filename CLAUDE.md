...
12. 🟡 Prompt iteration loop (A) — **checkpoint met**: a burst produced a 4,733-char root-cause report naming the machine, work order, product, operator and shift correctly. The warm-up misreading is **fixed at three levels**: the inference API now runs a throwaway prediction at startup (first request 2,861 ms → 71 ms), `get_recent_detections` returns an `inference_latency` block separating steady state from cold starts *and still flags genuinely widespread slowness as real*, and both prompt blocks forbid promoting an absence into a root cause. Open: a second failure mode seen 2026-08-02 — the agent invented the defect type `camera`, found none, and reported "integration failure" plus a "nomenclature mismatch" between `VisionDetections` and `Defects`, which are separate by design. Prompt rules now cover it; **not yet re-tested against a live paid run.**
