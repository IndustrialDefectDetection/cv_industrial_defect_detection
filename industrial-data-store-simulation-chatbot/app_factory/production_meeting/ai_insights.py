...
def provide_tab_insights(tab_name, dashboard_data=None):
    ...
    """
    logger.info("Providing tab insights for: %s", safe_log_text(tab_name))
    ...
    try:
        ...
        logger.info(
            "Successfully generated tab insights for %s",
        ...
        logger.error(
            "Error generating tab insights for %s: %s",
            safe_log_text(tab_name),
            safe_log_text(e),
        )
...
def generate_ai_insight(context, query=None, dashboard_data=None, model_id=None, temperature=0.1, include_historical=True):
    ...
    """
    logger.info(
        "Generating AI insight for context %s, query %s",
        safe_log_text(context),
        safe_log_text(query, max_chars=50) if query else "None",
    )
    ...
    try:
        with st.spinner("Generating AI insights using production meeting agents..."):
            ...
            logger.debug(
                ...
                safe_log_text(agent_query, max_chars=100),
            )
            ...
            try:
                ...
                logger.error(
                    "Async processing failed: %s",
                    safe_log_text(async_error),
                )
            ...
            if response.get('success', False):
                ...
                logger.info(
                    "Successfully generated AI insight for context: %s",
                    safe_log_text(context),
                )
                return output
            else:
        ...
        error_msg = str(e)
        logger.error(
            "Critical error generating insights with production meeting agents: %s",
            safe_log_text(error_msg),
        )
            ...
            if not critical_items.empty:
                ...
                for i, row in critical_items.iterrows():
                    with st.container():
                        ...
                        with cols[0]:
                            risk_icon = "🔴" if risk_color == "red" else "🟠"
                            st.markdown(f"**{risk_icon} {row['RiskLevel']} Risk**")
...
def display_cached_analysis(analysis_data, analysis_type):
    ...
    if analysis_type == "Quick Summary":
        ...
        if analyses:
            ...
            if tab_names:
                ...
                for i, (tab, data) in enumerate(zip(tabs, tab_data)):
                    with tab:
                        st.markdown(
                            safe_model_markdown(data["analysis"]),
        ...
        if target_analysis and target_analysis in analyses:
            ...
            if 'analysis' in analysis:
                st.markdown(
                    safe_model_markdown(analysis["analysis"]),
                    unsafe_allow_html=False,
                )
            else:
...
def display_ai_insights_tab():
    ...
    if hasattr(st.session_state, 'switch_to_chat'):
        ...
        try:
            ...
            logger.exception("MES Chat interface import failed")
            st.error("The MES Chat interface could not be loaded.")
            ...
            if agent_manager.is_ready():
                ...
                if st.button("Initialize Agents"):
                    try:
                        ...
                        logger.error(
                            "Production meeting agents failed to initialize: %s",
                            safe_log_text(exc),
                        )
                        st.error("The agents could not be initialized. Check the logs.")
    ...
    with col1:
        ...
        if hasattr(st.session_state, 'current_analysis') and use_cached == "Live Agent (Comprehensive)":
            if st.session_state.current_analysis == "summary":
                ...
                # Agent output is untrusted content. Streamlit's normal
                # Markdown renderer keeps raw HTML inert.
                st.markdown(
                    safe_model_markdown(summary), unsafe_allow_html=False
                )
...
def provide_contextual_tab_insights(tab_name, dashboard_data=None):
    ...
    if not agent_manager.is_ready():
        try:
            ...
            logger.error(
                "Failed to initialize agent manager for tab insights: %s",
                safe_log_text(e),
            )
